...
export default function NewGenreScreen() {
  ...
  const handleSubmit = async () => {
    if (!name.trim()) {
      Alert.alert(t('genres.new.name_required'));
      return;
    }
    setSubmitting(true);
    try {
      ...
      Alert.alert(t('genres.new.error'));
    }
    setSubmitting(false);
  };
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.card}>
        <Text style={styles.label}>{t('genres.new.name_label')}</Text>
        <TextInput style={styles.input} value={name} onChangeText={setName} placeholder={t('genres.new.name_placeholder')} placeholderTextColor="#666" />
        <TouchableOpacity style={styles.button} onPress={handleSubmit} disabled={submitting}>
          {submitting ? (
            <ActivityIndicator color="#00d4ff" />
          ) : (
            <Text style={styles.buttonText}>{t('genres.new.submit')}</Text>
          )}
        </TouchableOpacity>
      </View>
    </ScrollView>
